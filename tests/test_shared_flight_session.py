...
class TestClimbFailure:
    def test_climb_exc_airborne_safe_landing(self):
        s, c = _make_session()
        c.moveByVelocityAsync.side_effect = Exception("climb fail")
        c.getMultirotorState.return_value = _flying_st()
        with pytest.raises(SessionError, match="moveByVelocityAsync failed"):
            s.takeoff_and_climb()
        c.hoverAsync.assert_called()
        c.landAsync.assert_called()

    def test_climb_exc_on_ground_disarm_release(self):
        s, c = _make_session()
        c.moveByVelocityAsync.side_effect = Exception("climb fail")
        c.getMultirotorState.return_value = _landed_st()
        with pytest.raises(SessionError, match="moveByVelocityAsync failed"):
