...
def _make_reader(frame_timeout_seconds=0.5, monotonic_clock=None):
...
class TestTimestampStaleness:
    """Timestamp-based staleness detection with timeout."""
    ...
    def test_repeat_exceeds_timeout(self):
        clock = _FakeClock(1000.0)
        reader = _make_reader(frame_timeout_seconds=0.5, monotonic_clock=clock)
        ...
        raw2 = _make_mock_lidar_data([1.0, 2.0, 3.0], time_stamp=100)
        frame2 = reader._build_frame(raw2, clock())
        assert frame2.frame_valid is False
        assert frame2.invalid_reason == "stale"
        assert frame2.point_cloud_sensor.size == 0  # no usable data
    ...
    def test_new_timestamp_restores_validity(self):
        ...
        frame3 = reader._build_frame(raw3, clock())
...
class TestTimestampZero:
    ...
    def test_timestamp_zero_exceeds_timeout_stale(self):
        """Timestamp=0 repeated beyond timeout becomes stale."""
        clock = _FakeClock(1000.0)
        reader = _make_reader(frame_timeout_seconds=0.5, monotonic_clock=clock)
    ...
    def test_nonzero_timestamp_original_behavior(self):
        ...
        clock.advance(0.6)
        raw2 = _make_mock_lidar_data([1.0, 2.0, 3.0], time_stamp=500)
        frame2 = reader._build_frame(raw2, clock())
        assert frame2.frame_valid is False
        assert frame2.invalid_reason == "stale"
