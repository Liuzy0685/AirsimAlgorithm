...
class _StubPerceptionWorker:
    """No-thread PerceptionWorker stand-in for the deterministic fixed-goal
    tests.

    These tests patch ``time.sleep``/``time.monotonic`` to a single global fake
    clock, then drive ``AutomaticMode.run()`` on the main thread.  The real
    ``PerceptionWorker`` runs a daemon thread that also sleeps on that shared
    fake clock, which races the main loop and makes the iteration count
    non-deterministic.  This stub publishes one fresh snapshot synchronously
    (no thread, no fake-clock use) so the control loop never stale-hovers and
    the fake clock is advanced only by the main loop.
    """

    def __init__(self, lidar, perceive, poll_hz=10.0, **kwargs):
        self.poll_count = 0
        self.error_count = 0
        self._latest = None
        try:
            lf = lidar.read()
            fr, dd, rays = perceive(lf)
            from flight_modes.automatic_mode import PerceptionSnapshot
            self._latest = PerceptionSnapshot(
                lf=lf, fr=fr, dd=dd, rays=rays,
                received_mono=0.0, stale_count=0,
            )
            self.poll_count = 1
        except Exception:
            pass
    ...
    def snapshot_age_s(self, now=None):
        return 0.0 if self._latest is not None else float("inf")
...
class TestFixedMissionGoal:
    ...
    def _build_and_capture_goals(self, initial_yaw=0.0,
        ...
        for target in [_LR_G, _SR_G, _CR_G, _LPC_G, _FLT_G, _DD_G, _LFOV_G, _VFOV_G, _VC_G]:
            mocks.append(stack.enter_context(patch(target)))
        lr, sr, cr, lpc, flt, dd, lfov, vfov, vc = mocks
        # Isolate the fake clock: the real PerceptionWorker is a daemon thread
        # that would sleep on this same global fake clock and race the main
        # loop's iteration count.  A no-thread stub publishes one fresh snapshot.
        stack.enter_context(
            patch("flight_modes.automatic_mode.PerceptionWorker", _StubPerceptionWorker)
        )
        ...
        lr.return_value.read.return_value = _lf_g()
        ...
        # Disable the shadow replan cadence so plan_with_result runs every
        # frame — these tests sample the (start, goal) pair each iteration to
        # verify the mission goal is FIXED, independent of the A* rate-limit.
        auto._cbmba_replan_interval_s = 0.0
