...
class SharedFlightSession:
    ...
    def takeoff_and_climb(self, target_z: Optional[float] = None) -> None:
        ...
        try:
            self._client.moveToZAsync(z=self.target_z_ned, velocity=self.max_vertical_speed_mps,
                                       timeout_sec=30, vehicle_name=vn).join()
            logger.info("Climbed to Z=%.1f m.", self.target_z_ned)
        except Exception as e:
            self._handle_post_arm_failure(vn, "moveToZAsync", e)
