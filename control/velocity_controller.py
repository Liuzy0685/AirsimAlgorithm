...
class VelocityController:
    ...
    def send_velocity_body_frd(
        ...
        """Send body-frame FRD velocity.

        If ``yaw_rad`` is provided, converts body→world and uses
        ``moveByVelocityAsync`` (SimpleFlight does not support
        ``moveByVelocityBodyFrameAsync``).  Otherwise falls back to
        ...
        yaw_rad: Optional current yaw (rad) for body→world conversion.
        """
        if vehicle_name is None:
            raise VelocityCommandRejected("vehicle_name must be provided")
        ...
        if yaw_rad is not None:
            ...
            logger.debug(
                "moveByVelocityAsync: vx_world=%.3f vy_world=%.3f vz=%.3f dur=%.3f yaw=%.3f",
                vx_world, vy_world, vz, dur, yaw_rad,
            )
            return client.moveByVelocityAsync(
                vx_world, vy_world, vz, dur,
                drivetrain=a.DrivetrainType.MaxDegreeOfFreedom,
                yaw_mode=yaw_mode,
                vehicle_name=vehicle_name,
            )
        ...
        logger.debug(
            "moveByVelocityBodyFrameAsync: vx=%.3f vy=%.3f vz=%.3f dur=%.3f",
            vx, vy, vz, dur,
        )
    ...
    def send_velocity_body_frd_z(
        ...
        """Send body-frame FRD velocity with an altitude position hold.

        Uses ``moveByVelocityZBodyFrameAsync``: X/Y are body-frame velocities
        (forward/right), ``target_z`` is the **local NED altitude target** the
        AirSim controller holds.  This is an altitude *position* hold, unlike
        ``send_velocity_body_frd`` whose ``vz`` is a vertical *velocity*.

        Coordinate semantics:
            vx       = Forward (body X)
            vy       = Right   (body Y)
            target_z = Down-positive NED altitude to hold (e.g. -1.0)
        ...
        vx, vy, _ = self._validate_velocity(vx, vy, 0.0)
        dur = self._validate_duration(duration)
        yaw_rate = self._validate_yaw_rate(yaw_rate)
        ...
        logger.debug(
            "moveByVelocityZBodyFrameAsync: vx=%.3f vy=%.3f target_z=%.3f "
            "dur=%.3f yaw_rate_radps=%s",
            vx, vy, target_z, dur, yaw_rate,
        )
        return client.moveByVelocityZBodyFrameAsync(
            vx, vy, target_z, dur,
            drivetrain=a.DrivetrainType.MaxDegreeOfFreedom,
            yaw_mode=yaw_mode,
            vehicle_name=vehicle_name,
