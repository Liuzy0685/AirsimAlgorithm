...
class GoalTerminationChecker:
    ...
    def update(
        ...
        if self._goal is not None and self._goal != goal_ned:
            self.reset(goal_ned)
        ...
        if res.window_ready:
            ...
            res.position_std_m = math.sqrt(var)
        res.speed_low = (
            res.window_ready
        ...
        # Arrival requires the requested 3-D distance condition (A), recent
        # low velocity (B), recent position stability (C), and the explicit
        # altitude tolerance.  The component flags remain separate so the
        # runtime diagnostic can identify which condition blocked arrival.
        if (
