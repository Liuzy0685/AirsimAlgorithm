...
class LocalRecovery:
    ...
    def _detect_stuck(
        ...
        if len(self._window) < p.stuck_min_frames:
            # Window not full yet → no stuck verdict, but report the ACTUAL
            # latest/oldest position (never the (0,0,0) sentinel) so recovery
            # diagnostics and the position-source trace always see real motion.
            # (``update`` appends before calling this, so the window is non-empty.)
            _last = self._window[-1]
            _first = self._window[0]
            return (False, _last.timestamp - _first.timestamp, 0.0,
                    _first.px, _first.py, _first.pz,
                    _last.px, _last.py, _last.pz)
