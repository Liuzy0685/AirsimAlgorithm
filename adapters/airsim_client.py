...
class AirSimClientAdapter:
    ...
    def clone_readonly(self) -> "AirSimClientAdapter":
        """Return a new, **unconnected** adapter with the same connection params.

        Used by the background perception worker (Phase C2 sec 10-14) to own an
        independent read-only AirSim client, so its slow ``getLidarData`` RPC
        never contends with the control loop's client.  The clone is read-only
        (no control commands) and must be ``connect()``-ed before use.
        """
